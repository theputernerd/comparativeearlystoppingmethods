    ...
    for p in s:
        ...
        while not (wilsPredicted and baysPredicted):  # keep going until both methods made a prediction
            ...
            if not wilsPredicted:
                ...
                winner, method = shouldIStop(1, p1L, p1U, mean, epsilon=0,pmargin=0) #no threshold for lcb only
                if winner != 0:  #condition1
                    wilsPredicted = True
                    # now to see if prediction is correct.
                    if int(method) != int(1):
            ...
            if not baysPredicted:
                ...
                winner, method = shouldIStop(1, p1L, p1U, mean, epsilon=0,pmargin=0)
                if winner != 0:
                    baysPredicted = True
                    # now to see if prediction is correct.
                    if int(method) != int(1):
...
if __name__ == '__main__':
    ...
    fullResult=dict()
    alpha=0.05
    coverageTest(ngames=3000,drawThreshold=0.05,alpha=alpha,pmargin=0.8)
    choosefromPoolTest(ngames=3000,drawThreshold=0.05,alpha=alpha,pmargin=0.8)
